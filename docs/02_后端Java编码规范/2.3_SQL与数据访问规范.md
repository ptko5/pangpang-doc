...
// 动态SQL - 使用MyBatis动态标签
<select id="listUsers" resultType="UserPO">
    SELECT id, username, email
    FROM user
    WHERE 1=1
...
| 查询列表 | `selectList` / `selectPage` | `selectList(UserQueryDTO query)` |
| 新增 | `insert` | `insert(UserPO user)` |
| 更新 | `updateById` / `updateSelective` | `updateById(UserPO user)` |
| 删除 | `deleteById` / `deleteBatch` | `deleteById(Long id)` |
| 统计 | `selectCount` | `selectCount(UserQueryDTO query)` |
...
public interface UserMapper {
    ...
    List<UserPO> selectList(UserQueryDTO query);
    
    PageResult<UserPO> selectPage(UserQueryDTO query);
    
    int insert(UserPO user);
    
    int updateById(UserPO user);
    
    int updateSelective(UserPO user);
    
    int deleteById(Long id);
    
    int deleteBatch(List<Long> ids);
    
    int selectCount(UserQueryDTO query);
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public void createUserWithOrder(UserPO user, OrderPO order) {
...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void createOrder(OrderPO order) {
...
public class OrderServiceImpl implements OrderService {
    ...
    public void createOrder(Long userId, Long productId, Integer count) {
        ...
        orderMapper.insert(createOrderPO(userId, productId, count));
    }
}
```
