...
```java
// 使用虚拟线程处理批量IO任务
public CompletionStage<List<UserVO>> batchQueryUsers(List<Long> userIds) {
...
```java
// ✅ 正确：使用record模式匹配提取字段
public String formatUser(UserVO user) {
    return switch (user) {
        case UserVO(Long id, String name, String email, var status, var createdAt) ->
            String.format("User[%d]: %s (%s)", id, name, email);
    };
}

// ✅ 正确：嵌套record模式匹配
public String formatOrder(OrderVO order) {
    return switch (order) {
        case OrderVO(Long id, UserVO(Long userId, String userName, ..), BigDecimal amount) ->
